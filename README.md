...
This repo includes a [`render.yaml`](render.yaml) blueprint for a **single Web Service** (React build + Node API + WebSocket on one URL).

### 1. MongoDB Atlas

Use [MongoDB Atlas](https://www.mongodb.com/cloud/atlas) (free tier works). In **Network Access**, allow `0.0.0.0/0` so Render can connect (or restrict to Render egress IPs in production).

Copy your connection string — you will paste it as `MONGODB_URI` on Render.

### 2. Create the Render service

1. Open [Render Dashboard](https://dashboard.render.com/) → **New** → **Blueprint**
2. Connect GitHub repo: `tejasjeet/Cyber-Watch-India-AI-Threat-Intelligence`
3. When prompted, set these **secret** environment variables (from your local `server/.env`):

| Variable | Required |
|----------|----------|
| `MONGODB_URI` | Yes |
| `GOOGLE_AI_API_KEY` | For AI victim analysis |
| `GROQ_API_KEY` | For Live Feed streaming |
| `GOOGLE_SAFE_BROWSING_API_KEY` | For Phishing Scanner |

`JWT_SECRET` is auto-generated by the blueprint.

4. Click **Apply** — Render builds and deploys (~5–10 min first time).

Your live URL will be like: `https://cyber-watch-india.onrender.com`

### 3. After deploy

- Register a user at `/register`
- Log in — the app will refresh intel from MongoDB / scraper
- **Free tier** services spin down after ~15 min idle; first visit may take 30–60 s to wake up
...
Open `http://localhost:4000` — UI and API on the same port.
